    ...
    <div className="space-y-4">
      <Card title=" Linden Shooting Range" subtitle="Train accuracy. Burn ammo for a temporary buff that boosts your hit chance with ranged weapons.">
        <div className="grid sm:grid-cols-3 gap-3 text-xs">
          <div>
            <div className="text-[12px] uppercase text-ink-100/50">Accuracy</div>
            <div className="font-display text-2xl text-emerald-300">+{acc.current}</div>
            {acc.current > 0 && <div className="text-[12px] text-ink-100/50">fades in <Timer until={acc.fadesAt} /></div>}
            {acc.current === 0 && <div className="text-[12px] text-ink-100/50">no active buff</div>}
          </div>
          <div>
            <div className="text-[12px] uppercase text-ink-100/50">Equipped weapon</div>
            <div className="text-sm">{data.weapon ? data.weapon.name : <span className="text-blood-400">no ranged weapon equipped</span>}</div>
            {data.weapon && <div className="text-[12px] text-ink-100/50">ammo: {data.weapon.ammoType}</div>}
          </div>
          <div>
            <div className="text-[12px] uppercase text-ink-100/50">Ammo on hand</div>
            <div className="text-sm tabular-nums">{data.ammoOnHand} rounds</div>
            {!data.weapon && <Link className="btn btn-ghost text-[12px] mt-1" to="/">→ Equip a gun</Link>}
          </div>
        </div>
              ...
              return (
                <div key={d.id} className="rounded-lg p-3 border border-ink-100/10 bg-ink-950/40">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="text-[12px] text-ink-100/50">{d.energy} en · {d.ammo} rounds</div>
                  </div>
                  <div className="text-[13px] text-ink-100/55 mt-1">{d.desc}</div>
                  <div className="text-[12px] uppercase text-emerald-300 tracking-wide mt-2">+{d.buff} accuracy</div>
