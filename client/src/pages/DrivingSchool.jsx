...
export default function DrivingSchool() {
  ...
  async function load() { setData(await api.get('/driving-school')); }
  useEffect(() => { load(); }, [character?.driving, character?.cash]);

  async function study(course) {
    setBusy(course.id); setMsg(null);
    try {
      await api.post('/driving-school/study', { course_id: course.id });
      setMsg(`Completed ${course.name} — +${course.gain} driving (permanent).`);
  ...
  return (
    <div className="space-y-4">
      ...
      {data.maxed && (
        <Card>
          <p className="text-sm text-money-400 font-medium"> Mastered the wheel.</p>
          <p className="text-xs text-ink-100/60 mt-1">
            You're at the driving cap of {data.cap}. Race-edge bonuses and condition dampening are
            both maxed out; nothing left for the instructors to teach.
          </p>
        </Card>
      )}
