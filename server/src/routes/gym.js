...
// Each gym session also nudges a hidden progress counter. Cross 1.0 and the
// base stat permanently goes up by 1. Rate scales with the machine's buff
// for that stat — so a +3 strength bench earns more progress than dumbbells.
...
router.post('/train', requireAuth, requireCharacter, requireFreeCharacter, (req, res) => {
  ...
  // Permanent progression — accrue progress, tick base stat when full.
  // At or above the per-stat cap the temp buff still applied above, but no
  // further permanent gain accumulates and any leftover progress is dropped.
