...
// Starter cities are restricted so a brand-new character can't sit
// in a high-tier hub from second one and farm its multipliers.
// Higher-unlock cities show in the picker but are gated.
...
router.post('/create', requireAuth, (req, res) => {
  ...
  if (!isStartableCity(cityById(city))) {
    const c = cityById(city);
    return res.status(400).json({ error: `${c?.name || 'That city'} unlocks at level ${c?.unlockLevel || '?'} — pick a starter city instead.` });
  }
  ...
  writeLog(info.lastInsertRowid, 'system', `Welcome to ${cityById(city).name}, ${name}. No starter pack — earn what you keep.`);
...
router.post('/new-character', requireAuth, (req, res) => {
  ...
  writeLog(ch.id, 'system', `${trimmed} starts fresh — level ${startLevel}. Welcome to ${cityById(city).name}.`);
